namespace Nessos.Streams.Tests
    ...
    type ``ParStreams tests`` () =
        ...
        member __.``mapi`` () =
            Spec.ForAny<int[]>(fun xs ->
                let x = xs |> ParStream.ofArray |> ParStream.filter (fun x -> x % 2 = 0) |> ParStream.mapi (fun i n -> (i, n)) |> ParStream.toArray
                let y = xs |> PSeq.filter (fun x -> x % 2 = 0) |> PSeq.mapi (fun i n -> (i, n)) |> PSeq.toArray
                x = y).QuickCheckThrowOnFailure()
